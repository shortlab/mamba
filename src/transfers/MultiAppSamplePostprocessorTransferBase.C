...
InputParameters validParams<MultiAppSamplePostprocessorTransferBase>()
{
  ...
  params.addRequiredParam<std::vector<Point> >("position_offset", "The offset from the multiapp position. Specify '0 0 0' for to use the multiapp_position.");
  return params;
}
    ...
    case TO_MULTIAPP:
    {
      ...
      for (unsigned int i=0; i<_multi_app->numGlobalApps(); i++)
      {
        ...
        { // Get the value of the variable at the point where this multiapp is in the master domain
          ...
          std::vector<Point> point_vec(1, multi_app_position + _position_offset[0]); // offset the getValue call from the multiapp position
...
Real
MultiAppSamplePostprocessorTransferBase::getValue(MooseVariable & variable) const
{
      return variable.sln()[0];
}
