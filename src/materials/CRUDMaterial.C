...
// This parameter is the scaling factor, or the number to multiply by to scale from meters to something
  ...
  params.addParam<Real>("crud_thickness", 1, "The scaled thickness of the CRUD");
...
CRUDMaterial::CRUDMaterial(const std::string & name,
    ...
    :Material(name, parameters),
     ...
     _t_crud(getParam<Real>("crud_thickness")),
...
//  if (_HBO2[_qp] < 0.5)
  _k_liquid[_qp] =
      0.686
    + ((7.3e-10) * ((isParamValid("pressure") ? _P[_qp] : 15000000) / _scaling_factor))
...
/***********************************************************/
  ...
  _mu_h2o[_qp] =
      25.3
    / (-8.58e4 + (91 * _T[_qp]) + std::pow(_T[_qp], 2));
  ...
  _h_fg_h2o[_qp] /= std::pow(_scaling_factor, 2);	// Scale inverse twice for m^2 (kJ/kg = kg*m/s^2 * m * 1/kg)
  ...
  _cp_liquid[_qp] /= std::pow(_scaling_factor, 2);	// Scale inverse twice for m^2 (kJ/kg-K = kg*m/s^2 * m * 1/kg-K)
